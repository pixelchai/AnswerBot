    ...
    for query in parse_question(question):
        ...
        for variation in variations:
            ...
            for page in pages:
                ...
                x=search_data(variation,[(0.0,x) for x in page[2].sents]) # skip first grouping - that decided the page itself
                yield x
                # print(
                #     x
                # )
...
def search_data(variation, spans,limit=10):
    ...
    for span in spans:
        ...
        group_str = ' '.join(str(x) for x in group)
        sim = span[1].similarity(nlp(group_str))
        # print(sim)
        ret.append((sim+span[0], span[1]))
    # only consider top x
    ret.sort(key=lambda x:x[0],reverse=True)
    ret=ret[:limit]
    if len(variation)>2:
...
if __name__=='__main__':
    result=list(search('the biggest animal of Europe'))
